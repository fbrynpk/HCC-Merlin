...
class CTPersistentDataset(monai.data.PersistentDataset):
    ...
    def _cachecheck(self, item_transformed):
        ...
        if hashfile is None:
            return _item_transformed
        try:
            # NOTE: Writing to a temporaJry directory and then using a nearly atomic rename operation
...
class VerseDataLoader(monai.data.DataLoader):
    def __init__(
        self,
        datalist: List[dict],
        cache_dir: str,
        ...
        self.dataset = CTPersistentDataset(
            data=datalist,
            transform=VerseImageTransforms,
            cache_dir=cache_dir,
        )
        super().__init__(
            self.dataset,
